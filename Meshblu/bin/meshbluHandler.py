...
class ConfigApp(admin.MConfigHandler):
  ...
  def setup(self):
    if self.requestedAction == admin.ACTION_EDIT:
      for arg in ['meshblu_server', 'server_uuid', 'server_token']:
  ...
  def handleList(self, confInfo):
    confDict = self.readConf("blusetup")
    if None != confDict:
      for stanza, settings in confDict.items():
        for key, val in settings.items():
          if key in ['meshblu_server', 'server_uuid', 'server_token'] and val in [None, '']:
            val = ''
...
write them to the [setupentity] stanza
    ...
    self.writeConf('blusetup', 'config', self.callerArgs.data)
